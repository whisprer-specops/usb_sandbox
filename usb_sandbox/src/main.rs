...
async fn main() -> Result<()> {
    ...
    println!("🎯 Ready to analyze USB devices, husklyfren!");
    println!("💡 Commands:");
    println!("   'analyze' - Analyze nixie clock");
    println!("   'monitor' - Start real-time USB monitoring");
    println!("   'quit' - Exit the sandbox");
    println!("🔌 Waiting for input... (type 'help' for commands)");
        ...
        match tokio::io::AsyncBufReadExt::read_line(&mut reader, &mut line).await {
            ...
            Ok(_) => {
                ...
                match command.as_str() {
                    "analyze" | "nixie" => {
                        println!("🚀 Running REAL analysis on nixie clock...");
                        if let Err(e) = real_usb_analyzer::run_real_usb_analysis().await {
                            println!("❌ Real analysis failed: {}", e);
                        }
                    }
                    "monitor" | "watch" => {
                        println!("👀 Starting USB monitoring mode...");
                        println!("   Plug in your nixie clock now!");
                        ...
                        for i in 1..=30 {
                            ...
                            if i % 5 == 0 {
                                println!("   📡 Monitoring... {}s", i);
                            }
                        }
                        println!("   ⏹️ Monitoring stopped");
                    }
                    "protocol" | "deep" => {
                        println!("🔬 Starting REAL USB protocol analysis...");
                        if let Err(e) = crate::real_usb_analyzer::run_real_usb_analysis().await {
                            println!("❌ Real analysis failed: {}", e);
                        }
                    }
                    "packets" | "capture" => {
                        println!("📦 Starting raw packet capture...");
                        println!("   Capturing USB traffic for 15 seconds...");
                        
                        // This would integrate with real USB capture
                        for i in 1..=15 {
                            tokio::time::sleep(Duration::from_secs(1)).await;
                            if i % 3 == 0 {
                                println!("   📡 Captured {} packets so far...", i * 3);
                            }
                        }
                        println!("   📄 Packet capture saved to raw_data/");
                    }
                    "help" | "h" => {
                        println!("📋 Available commands:");
                        println!("   analyze   - REAL analysis of connected nixie clock");
                        println!("   monitor   - Monitor for device plug/unplug events");
                        println!("   protocol  - Deep protocol reverse engineering");
                        println!("   capture   - Live USB traffic capture (60s)");
                        println!("   scan      - Scan for all USB devices");
                        println!("   status    - Show sandbox status");
                        println!("   quit      - Exit the sandbox");
                        println!("   help      - Show this help");
                    }
                    "scan" | "devices" => {
                        println!("🔍 Scanning for USB devices...");
                        let analyzer = real_usb_analyzer::RealUsbAnalyzer::new("8087", "0AAA");
                        if let Err(e) = analyzer.scan_usb_devices().await {
                            println!("❌ USB scan failed: {}", e);
                        }
                    }
                    "status" | "s" => {
                        println!("📊 Sandbox Status:");
                        println!("   🟢 Sandbox: Active");
                        println!("   📁 Output: {}", manager.reporter.output_dir.display());
                        println!("   🔧 VM Type: {:?}", manager.config.vm_type);
                        println!("   💾 Memory: {}MB", manager.config.memory_limit_mb);
                    }
                    "quit" | "exit" | "q" => {
                        println!("👋 Shutting down USB sandbox...");
                        break;
                    }
                    "" => {
    ...
    println!("🏁 USB Sandbox shutdown complete. Goodbye, fren!");
